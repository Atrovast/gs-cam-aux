### colmap to nerfstudio 'transforms.json'
import os
import sys
import numpy as np
import json
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, alt, scale):
    ...
    scene1 = sys.argv[2]
    l1 = os.listdir(os.path.join(f'/home/dsh/datasets/ScanNet/{scene1}/test/images'))
    # print(l1)
    # l2 = [os.listdir(os.path.join(f'/home/dsh/datasets/lerf-ovs/label/{scene1}')) for i in l1]
    l3 = [y.split('.')[0] for y in l1 if y.endswith('.jpg')]
...
if __name__ == "__main__":
    readColmapSceneInfo(sys.argv[1], sys.argv[3])
